...
**추가 이유**: 여러 화면의 텍스트 레이어에서 같은 크기 · 굵기 조합이 반복되어, 값이 어긋나지 않도록 한 곳에 모았습니다.
Figma에 의미 기반 타입 스케일이 없으므로 이름은 `굵기 + 크기`로만 붙였고, 색상은 쓰는 곳에서 `copyWith(color: ...)`로 지정합니다.
두 곳 이상에서 쓰는 조합만 올립니다. 한 곳에서만 쓰는 스타일은 그 위젯에 둡니다.

행간은 px로 적었습니다. Flutter의 `height`는 글자 크기에 곱하는 배수라서 코드에서는 `행간px / 크기`(예: `22 / 19`)로 씁니다.
값은 과제 1 화면의 텍스트 레이어에서 그대로 옮겼습니다. Figma에 없는 조합(12 · 14 · 16px 등)은 만들지 않습니다.

| Dart | 크기 | 굵기 | 행간 | 자간 | 쓰는 곳 |
|---|---|---|---|---|---|
| `AppTypography.regular11` | 11 | regular | 14 | 0 | 종목코드 · 시장, 등락, 탭 라벨, 빈 상태 설명, 표 셀 |
| `AppTypography.regular13` | 13 | regular | 18 | 0 | 기간 칩 |
| `AppTypography.medium15` | 15 | medium | 20 | -0.1 | 종목명, 가격, 검색어, 정렬 옵션 |
| `AppTypography.bold13` | 13 | bold | 18 | 0 | 정렬 버튼, 섹션 제목, 토스트 |
| `AppTypography.bold19` | 19 | bold | 22 | -0.2 | 헤더 · 빈 상태 · 바텀시트 제목 |

현재가(Bold 30 / 36 / -0.4)는 한 곳에서만 써서 `price_header.dart`에 둡니다.
